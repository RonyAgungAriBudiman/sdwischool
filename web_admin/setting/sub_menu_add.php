...
if(isset($_POST['Simpan']) or isset($_POST["Update"])){
	...
	if (count($pesanError)>=1 )
		...
		{
		//CREATE NO
		$mysql		= mysql_query("SELECT  pg_no from M_MENU where mn_no = '$mn_no' order by pg_no desc", $koneksidb);
		...
		$mysql  	= "INSERT INTO M_MENU (mn_no,sm_no,pg_no,pg_name,
						...
						VALUES ('$mn_no','$sm_no','$sm_no','$pg_name','$nourut',
...
?>
					...
					<h3 class="blank1">ADD SUB MENU</h3>
...
<div class="panel-body panel-body-inputin">
		  <form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
					...
					<input id="sm_no" name="sm_no" type="hidden" value="<?php echo $sm_no; ?>"   readonly="" class="form-control1"/>
					 ...
					 <select class="form-control" id="mn_no" name="mn_no"  <? if ($Edit == "Edit") { echo "disabled = disabled";} ?> >
						...
						<option value="<?php echo $mn_no; ?>"  selected> <?php echo $pg_name; ?></option> 
								  ...
								  <?php				
									...
									$mn_no		=$row['mn_no'];
									$pgname		=$row['pg_name'];		
									print "<option value='$mn_no'>$pgname</option>";
									}
									?>
				</select>
				</div>
			...
			<div class="form-group mb-n">
				<label class="col-md-2 control-label">NAMA SUB MENU</label>
				<div class="col-md-8">
